...
**Current stage:** **Stage 6 — Chat Loop** (next to build). Stage 5 — Scene /
Persona / Scenario / Event Builders is **DONE-HERE** (2026-07-13; all [HERE]
DoD met — builders, the code-anchored consent gate, scene prompt assembly,
compositing, wiring, UI; 921 tests passing) and **hardware-VALIDATED
2026-07-13** on the RTX 4070 Super 12 GB (background generation + Layer-2 +
compositing + toggle; ~10.6 s/frame steady, ~11.9 GB peak). A bounded matte
weakness on tight close-up busts folds into the open 3f residual (BiRefNet
escalation). Stage 4 — Library &
Management is **DONE** (done-here 2026-07-13; the two deferred items it owned —
disk thresholds/LRU cap and the startup reconciliation sweep — are RESOLVED;
regeneration/eviction wiring is structural + unit-verified, actual regeneration
invocation rides Stage 3 on hardware). Stage 3 — Image Pipeline is
**COMPLETE**: 3a–3g all done-here AND hardware-validated on the RTX 4070 Super
12 GB target, with the named residuals below.
...
**Next action (when resumed):** **begin Stage 6 — Chat Loop** (split 6a–6e:
model load/swap manager → RAG memory store → decay model → persona injection +
turn assembly → avatar-frame selection). Stage 5 is **DONE-HERE**: the four
builders (persona/scene/event/scenario) produce valid records via the same
input model, a scenario carries a **code-anchored structural consent frame**
(Layer 3), scenes generate a background via the same image pipeline
(**[HARDWARE]**, Layer-2-screened), and a matted character composites over it
with the on/off toggle (**[HERE]**, real Pillow). Stage 6e (avatar-frame
selection) will *consume* `ImageService.composite_frame` — the primitive it
needs now exists. **Stage 5 carries a pending hardware flag** on background
generation + Layer-2 (see the flags list); the compositing + builder logic is
fully verified here (921 tests).
