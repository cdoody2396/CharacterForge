...
**Next action (when resumed):** TWO TRACKS, hardware validation first.
**(a) Hardware-validation continuation** (3a + 3b VALIDATED; 3c
mostly-validated post-CCIP-swap with a confirmed **20-frame vetted set on
disk** for character `c517663a…` — the 3d input, kept for exactly this):
clone kohya sd-scripts + wire `lora_trainer_dir`/`lora_trainer_python` →
run §13 3d training on the vetted set (**THE 12 GB stress test** — the §16
quality-max defaults were sized against 16 GB; expect to tune
dim/resolution/batch if fp16+gradient-checkpointing doesn't fit) → §15 3e
catalog with the trained LoRA → finish the remaining §17 3f items
(edge-quality tuning over composite backgrounds, purge drill, lifecycle) on
that real catalog. Two 3c items stay open by design: the Layer-2
false-negative recall check (user-directed) and the optional face-swap leg.
**(b) Then begin Stage 3g** — on-demand generation + cache: novel states
generate on demand, run the same auto-filter, matte via the 3f `Matter`, and
cache into the growing per-character library (`cache/`, `on_demand=true`,
§7).
