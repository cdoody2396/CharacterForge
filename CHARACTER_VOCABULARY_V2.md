...
2. **Required set unchanged.** The 7 required/quick group ids (`race`, `gender_presentation`, `skin_tone`, `hair_color`, `hair_style`, `eye_color`, `body_type`) are kept **byte-identical as ids** — only labels and option lists change (data-only). No construction-gate contract change. All new groups are `required:false`. *(5.7 amendment, user sign-off 2026-07-18: the 7 ids stay byte-identical, but requiredness became **required-when-visible** — `skin_tone`/`hair_style`/`hair_color` are conditional on the new required+quick `skin_type` surface / `hair_length`; see flag 10.)*
...
10. **5.7 sixth §15 extension + surface model (user sign-off 2026-07-18, post-V2).** (a) `visible_when` gains `not_in` — negative predicates read VISIBLE on an empty selection (quick-create satisfiability depends on this polarity). (b) The 5.6a "required must be unconditionally visible" rule is DELETED: **required-when-visible** — the construction gate evaluates `OptionCatalog.required_group_ids_for(selections, tags)` via the server-side `visible_now` evaluator (byte-matching creator.js `visibleNow`; one-hop non-recursive), and condition-hidden payload values are dropped server-side (a `validate_against` lint covers hand-edited records). (c) Additive `hint` field (group + option level, display-only). (d) The **`skin_type` unified surface group** (required+quick) gates `skin_tone` and re-keys the fur/scale/feather/chassis/ethereal color groups off race `class`; `fur/scale/feather_coverage` retired (their prompts live on the surface compounds); `glow_color` deliberately stays race-keyed. (e) `record.labels` — free-form, Layer-1-gated organizational labels (additive, schema_version unchanged). (f) `apparent_age` is quick + auto-defaulted from the entered age on create only (reopens the 2026-07-16 plain-optional decision); `archetype` relabeled "Class" (id untouched); new chat-side `personality_archetype`. Conditional chains are capped at ONE hop (the only shipped chain: `hair_color_pattern` → `hair_color_2` → `hair_length`) — deeper chains would break client/server convergence.
