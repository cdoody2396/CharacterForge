...
#### 5.5g — Promoted residuals  **[HARDWARE]**  ✅ **DONE-HERE 2026-07-14** ([HARDWARE] acceptance run + escalation/A/B/3c drills PENDING on the target)

**Outcome — all [HERE] code MET (done-here; 1015 tests passing, 1 skipped; +11 over 5.5d/5.5e; adversarially reviewed — red-team + correctness + DoD, 0 surviving findings):**
- **3f BiRefNet escalation — built, byte-for-byte no-op when unconfigured.** New `models.image.matting_escalation_model_path` (a second, user-placed BiRefNet `.onnx`) + `image_gen.matting.escalation_variant` (default `birefnet`) + `escalation_coverage` (default `0.85` — sits above the ~0.28 clean-frame ceiling and below `coverage_max` 0.98, so in-band busts 0.93–0.98 that currently *pass* the gate still escalate). `matte.py`: `MatteConfig.model_path` seam (default None ⇒ the factory loads `matting_model_path`, byte-identical), `EscalationConfig` + `coerce_escalation_config` (returns None **only** when the path is UNSET — the no-op guarantee, and unset≠misconfigured; junk hand-edits degrade to defaults, never raise), `matting_escalation_model_path`. `service.py`: `_MatteEscalation` (owns the escalation toolkit, built **lazily** on the first bust so a wide-frame-only catalog never loads the ~973 MB model; missing/corrupt model → disabled, never raises; `_build_escalation` also wrapped never-raise), and the DRY `_apply_escalation` seam — after the primary matte, a frame with primary coverage ≥ `escalation_coverage` is re-matted with BiRefNet and the escalated cutout is promoted **only if it passes the SAME `evaluate_matte` gate AND keys strictly more out (lower coverage)** than the primary (the never-worse rail). Wired into all matte paths (`matte_catalog`, `_matte_one` → `_heal_matte` + cache-gen); esc temp `<stem>.esc.png.tmp` is reaped by the existing `*.png.tmp` sweep and cannot collide with the primary tmp/final; `esc.close()` on every exit including the mkdir-fail early return. Provenance (`manifest.matting`: `escalation_variant`/`escalation_model` basename-only/`escalated` count) written **only when configured** (unconfigured manifest byte-identical). **Layers 1–4 unchanged** — the escalation re-mattes the SAME `src_abs` that already passed the Layer-2 classify-before-skip gate. 8 new matte tests (promotes-lower / rescues-`matte_full` / keeps-primary(not-better + fails-gate) / below-threshold-skips / not-configured-no-second-build / missing-model-degrades / lazy-build-once / pure-coerce-degrade).
- **3g `catalog_states.json` — full canonical-tag rewrite** (user-approved): `neutral`→`expressionless`, `smile`→`smile`, `laugh`→`laughing`, `serious`→`serious`, `surprised`→`surprised`, `sultry`→`seductive smile, half-lidded eyes`, `portrait`→`upper body, portrait, facing viewer`, `over_shoulder`→**`looking back`** (the confirmed defect); `standing`/`sitting` already canonical; `three_quarter` kept (no canonical Danbooru equivalent). Data-only, drop-in-editable (§15) — no code parses the string. New test asserts **every shipped fragment still passes the Layer-1 gate at assembly**; `test_resolve_cell_happy_path` updated (`gentle smile`→`smile`).
- **5.5b chunking-off toggle — the [HERE] gap that made 5.5b's A/B unrunnable.** `engine.encode_chunked(..., chunked=True)`: `chunked=False` does one `encode_prompt` over the raw strings (the pre-5.5b truncate-at-77 path) and returns the same 4-embed dict; `GenerationRequest.chunked` (default True) threaded from the new `image_gen.encode_chunked` setting via `_generation_settings` (`… is not False` — only an explicit boolean `false` disables; missing/junk stays chunked). All three backends pass `request.chunked`; default True is byte-identical. Surfaced in `engine_status().generation`. Test asserts `chunked=False` → single encode.
- **3c safety residuals — code paths already built + tested; the residual is the [HARDWARE] drill.** Added a sandbox-runnable RECALL anchor driving the REAL `_ImgutilsContentClassifier` matching logic against the REAL `minor_coded_tags.txt` (faked WD14 tagger): a minor-coded tag ≥ `MINOR_TAG_CONFIDENCE` blocks; below-threshold and clean frames don't. `minor_coded_tags.txt` stays the tuning surface; face-swap leg stays default OFF.

**REMAINING — the full [HARDWARE] acceptance run on the RTX 4070 Super 12 GB** (user-driven, no script; see the Stage DoD + the plan runbook): create a *fully-detailed* character → avatar → pick reference → bootstrap (Cancel mid-run) → approve the machine-vetted grid → train (Cancel mid-run; hard-kill → next-boot reap) → catalog → matte (place a BiRefNet `.onnx`; confirm `escalated>0` and the translucent bust pane is gone) → novel pose → composite over a scene, **entirely from the window**, progress polling ~1 Hz. Plus the 5.5b A/B (`encode_chunked` true vs false, same record+seed) and the 3c minor-appearance recall + blocked-frame purge drills. This single run clears the pending **5.5a** (timing/cancel/reap), **5.5b** (chunking A/B), **5.5d** (window-driven pipeline), and **3f-escalation** flags.
...
- **5.6a — §15 fifth format extension [HERE].** ✅ **DONE-HERE 2026-07-16.** Group `visible_when` (data-driven conditionality —
  ...
  **Outcome — all DoD MET (1073 tests passing, 1 skipped; adversarially verified, see change log):**
  `visible_when` load-normalized (`_coerce_visible_when`: `{"group": id}` + exactly one of
  `any:true | in:[ids] | class:"c"`; unparsable → `None` = always-visible, the doc's degrade
  semantics — never a format error; forbidden on a `required` group, a load-time error on both
  paths); `tier` closed-enum `P0|P1|P2|P3` (`_coerce_tier`, unknown → `OptionFormatError` like the
  ...
  (**default OPEN — user decision 2026-07-16**; `content_gate_open` coerces `is True`, junk/raising
  gate reads CLOSED), `CreatorService(gated_option_dirs, gate)` re-evaluates on every (re)load
  (dirs: bundled → `data/options` → `app/data/options_gated` → `data/options_gated`), thumbnail
  bases gated the same way, `build_creator(…, settings)`; the assembler sees the same gate-shaped
  catalog via the existing `lambda: creator.catalog` — structural, never a filter.
  `shell.py` `_SETTABLE` + `_BOOL_KEYS` admit the key (bool-only); Settings-view checkbox saves
  then reloads options via the newly exported `Creator.reloadOptions()` (returns true/false —
  the failure path reports honestly). Front-end: `condIndex`/`visibleNow`/`selectionChanged` in
  `creator.js` — re-render only when a condition-referenced group changes (zero re-renders on a
...
- **5.6b — Assembler tier ordering [HERE].** ✅ **DONE-HERE 2026-07-16.** Tier-bucketed stable sort replacing the flat
  ...
  load-bearing).
  **Outcome — all DoD MET:** `_assembly_groups` (`prompt.py`) sorts by
  `(_TIER_RANK.get(tier, 4), order, id)` — P0→P1→P2→P3→untiered, stable `(order, id)` within a
  ...
  and the 6-hex LoRA trigger below `boundary_index`. `img:—` = existing `render:false`;
  `img:gated` falls out of the gated catalog structurally (tested); `img:scene` stays
  `assemble_scene`'s.
- **5.6c — Subset A+B data authoring [HERE, render-side]. ✅ DONE-HERE 2026-07-16.** The 112-race
  ...
  + a boundary cross-product + maximal per-family end-to-end records; slider removal (data-only —
  height/weight/muscle groups deleted, the numeric machinery stays dormant still guarding `age`);
  the **DECISIONS §12 + §17 amendment** applied with explicit user approval; legacy slider/old-id
  records load leniently + lint (VERIFIED, not rebuilt). Real-tokenizer P0+P1 first-window contract
  re-run against the REAL authored catalog (lamia/harpy/ghost/android). **Outcome — all DoD MET:**
  ...
  `genitalia`/`genitalia_size`/`aesthetic`; `archetype` kind tags→single) with legacy values
  lint-only; `age` group `prompt_ranges` **removed** (V2 A3 renders via `apparent_age` only —
  reversible at 5.6e if renders regress); `horns`/`wings`/`faceplate` authored **P2** not P1
  (first-window budget; V2 B23 already allowed the split — 5.6e revisits). **Note for a future
  `prompt.py` hardening session (NOT 5.6c — pre-existing code, data-only scope):** `_gate_adjacency`
  excludes free-text pieces from its zero-separator concat pass, so the option↔`appearance_notes`
  boundary isn't zero-sep gated (reproduces with an EMPTY catalog — pre-existing, not a 5.6c
...
- **5.6d — Subset C+D data [HERE, render:false]. ✅ DONE-HERE 2026-07-17.** The 64 chat-side
  render:false groups (~1,247 options) split into `50_mind.json` (C1–C19), `55_speech.json`
  (C20–C31), `70_life.json` (D1–D34); the pre-V2 `50_personality.json` deleted (disposition/voice
  retire → lint; `traits` id **reused** by C7, only legacy value `intellectual` lints). The
  **free-text swap** (the one code change): `FREE_TEXT_FIELDS` → the four slots
  `nickname`/`signature_note`/`catchphrase`/`companion_name` (+ `name` = the five §10 slots);
  `IMAGE_FREE_TEXT_KEYS = ("signature_note",)`; `character.py` `_rehome_legacy_free_text`
  (`appearance_notes`→`signature_note` on load, signature_note wins if both, legacy note still
  renders; `personality_notes`/`backstory` left inert per keep+lint). **Decisions realized:** C19
  `signature_skill` = a full-list single picker duplicating C18's 72 ids with `visible_when
  {group:"skills", any:true}` (no format widget for options-from-another-group's-selection; `skills`
  stays unconditioned); `companion_name` **always-show** (data-only — no front-end change, the swap
  is the only code touch); provisional natural-language fragments carried on every option (re-cut at
  6d, flag 1); caps + single-select defaults documented in `_note` only (still unenforced — no
  format key). **Three Layer-1-forced data rewords** (doc-faithful label kept, id/fragment made
  safe on the option-id gate channel): occupation `con_artist`→id `grifter`, origin_story
  `street_kid`→id `street_urchin`, `only_child`→id `only_one` (fragment "raised without siblings")
  — found by a whole-C/D option-id + fragment Layer-1 pre-scan in strict `prompt` context. Count
  reconciliations (enumerated list authoritative): quirks 36, occupation ~151, fav_food 35.
  **Outcome — all DoD MET:** suite **1100 passing, 1 skipped (+5)**; catalog loads strict with zero
  errors; every C/D option id clears the strict prompt-context record gate; no render:false fragment
  reaches the image; only `creator.py`/`prompt.py`/`character.py` changed in production code
  (git-diff-confirmed — `options.py`/`catalog.py`/`catalog_states.json`/the 5.6c A+B data/gated
  files/`safety/**`/`creator.js` all untouched); required-7 + age gate untouched; D-vi builder
  vocabulary NOT authored. Adversarially verified (5-stream: correctness-vs-doc ×3 + red-team +
  DoD/scope, skeptic-refutation mandate) — **0 findings**.
- **5.6e — [HARDWARE] render pass. ✅ DONE-HERE 2026-07-17.** All three open-item-7 weak-honor
  fields judged on isolated ablation renders (near-human + non-standard frames) and **kept** by the
  user (each a harmless no-op, none actively hurts): `face_shape` (P3 — square/angular/long all
  render the same soft anime face), `nose` (P3 — aquiline/broad render the same minimal anime nose),
  `height_band` extremes (P2 — identical naga scale across the whole band; species-relative with no
  scale reference in a solo render, but retained for the chat-side `mem:inject` physique line). Both
  5.6c demotions **confirmed** on renders: `horns`/`wings`/`faceplate` render prominently at P2 even
  behind a full P1 identity load (re-promotion would cost first-window budget for zero visible gain),
  and `apparent_age` alone renders the age progression (the age-group `prompt_ranges` removal holds;
  the old end compresses by anime style, which restoring wouldn't fix). **Net: ZERO render-side data
  edits — every 5.6c call validated by pixels; open item 7 resolves as keep-all-three.** Full V2
  re-acceptance on a naga hero (non-standard serpent-coil frame, the hardest case) drove the whole
  pipeline — create → avatar → reference → bootstrap (30 vetted, sim 0.92–0.96) → confirm 30 →
  train (dim 16, 1600 steps, 6-hex trigger `0c3129`, 85 MB) → catalog (20, identity holds across
  expr×pose) → matte (20/20 clean, 0 escalated, 0 purged) → on-demand pose → composite over a
  generated background — **image quality user-approved** (5.5-closure discipline; evidence from the
  run's own `data/jobs`/`bootstrap.json`/`lora.json`/`catalog.json`/`audit-*.jsonl`). Carried drills
  cleared: the 5.5b chunking A/B (a tail `signature_note` at token ~92 renders ONLY chunked, absent
  when `encode_chunked` is false and diffusers truncates at 77) and the 3c minor-recall (cull screened
  64 candidates, 0 wrongly blocked; test anchor proves it catches minors) + blocked-frame purge (matte
  re-screened 20 frames, 0 purged), all on real data. Adversarially verified (git-scope + change-log
  accuracy + budget/Layer-1 red-team). Zero code/data/blocklist change — the only repo edit is this
  doc.

**Authoring notes carried to 5.6c/5.6d (from the 5.6a/5.6b verification):** (1) gated files load
AFTER every ungated directory — author them as append-only fragments (new groups / appended
options), never scalar overrides of ungated groups; (2) `visible_when` conditions should reference
always-visible groups (the required 7, or unconditioned groups — V2's conditions all read `race`,
...
degrade to always-visible by design (selections-only semantics — moot after the 5.6c slider
removal); (4) `app/data/options_gated/` ships README-only until 5.6c — the Settings toggle is
...
adversarially verified — correctness 0 mismatches, DoD/scope PASS, red-team 1 finding
triaged as pre-existing `prompt.py` code + out of the data-only scope, skeptic-confirmed):**
the Subset A+B render-side vocabulary — the 112-race file with `class` metadata + `hybrid_race`,
`apparent_age`/`age_reality`/`gender_identity`/`pronouns`, `archetype` grown to 34; all Subset B
groups (skin 28 / hair 25×2 / eye 17×2 swatches, the species-feature blocks conditioned on race
...
append-only, default gate OPEN. `DECISIONS.md` §12 + §17 amended with explicit user approval
...
real-catalog first-window contract beside the 5.6b fixture. **Session 3 (2026-07-17) shipped 5.6d
DONE-HERE (1100 tests passing, 1 skipped, +5; adversarially verified 5-stream — 0 findings):** the
Subset C+D chat-side render:false vocabulary — 64 groups / ~1,247 options in `50_mind.json`
(C1–C19), `55_speech.json` (C20–C31), `70_life.json` (D1–D34); `50_personality.json` deleted
(disposition/voice → lint, `traits` id reused by C7); the free-text swap (`FREE_TEXT_FIELDS` →
`nickname`/`signature_note`/`catchphrase`/`companion_name`; `IMAGE_FREE_TEXT_KEYS =
("signature_note",)`; `character.py` legacy `appearance_notes`→`signature_note` mapping) — the ONLY
code change, `options.py`/`catalog.py`/`catalog_states.json`/the 5.6c data/`creator.js` untouched.
C19 `signature_skill` = full-list picker conditioned on `skills`; `companion_name` always-show
(data-only); provisional fragments carried; caps/defaults still `_note`-only. Three Layer-1-forced
id rewords (grifter/street_urchin/only_one). **Session 4 (2026-07-17) shipped 5.6e DONE-HERE — the
[HARDWARE] render pass, closing Stage 5.6:** the three open-item-7 weak-honor fields (`face_shape`,
`nose`, `height_band` extremes) judged on isolated ablation renders and **kept** by the user (each a
harmless no-op; `height_band` retained for its chat `mem:inject`); both 5.6c demotions **confirmed**
on pixels (`horns`/`wings`/`faceplate` render fully at P2; `apparent_age` alone renders age) — **net
ZERO render-side data edits, every 5.6c call validated, open item 7 = keep-all-three**; a full V2
re-acceptance on a naga hero (non-standard serpent-coil frame) drove create→avatar→bootstrap (30 @
sim 0.92–0.96)→train (dim16/1600 steps)→catalog (20)→matte (20/20 clean)→pose→composite with **image
quality user-approved**; the carried 5.5b chunking A/B (tail fragment renders only chunked) and the
3c minor-recall + blocked-frame-purge drills **cleared** on real data. Only `BUILD_PLAN.md` changed
(git-diff-confirmed — zero code/data/blocklist edits; suite green 1100/1). **Stage 5.6 COMPLETE →
Next: Stage 6.**
**Stage 5.5 — Creator & Image UI is COMPLETE (2026-07-15)**:
...
fix built in 5.5g, pending the target run). Stage 6 — Chat Loop follows the acceptance run.
...
- **Stage 5.5b** — **CLEARED at 5.6e (2026-07-17).** Token count validated here; chunked encoding
  carried the re-run's renders. The explicit A/B ran on a maximal V2 record (total 92 content
  tokens, boundary at piece 24): same record + seed 42, `encode_chunked` true vs false — the tail
  `signature_note` ("a bright crimson rose", token ~92, past the 77 window) renders ONLY in the
  chunked pass and is truncated away in the non-chunked pass, everything else shared. Chunking drops
  nothing. The shortened 6-hex trigger is re-proven on real weights (the 5.6e naga LoRA-steered
  catalog holds identity).
...
- **Stage 3f escalation (5.5g)** — **VALIDATED 2026-07-15**: `birefnet-general.onnx`
  ...
  provenance recorded. The 3c minor-appearance recall + blocked-frame purge drills are **CLEARED at
  5.6e (2026-07-17)**: on the naga acceptance run the cull screened 64 bootstrap candidates with 0
  wrongly blocked (all adult — `test_bootstrap_cull.py::test_real_classifier_recall_against_the_minor_tag_file`
  proves it catches above-threshold minor-coded tags), and the matte re-screened 20 catalog frames
  fail-closed with 0 purged (the `test_matte.py` blocked-frame-purge anchors prove the purge path).
...
- **Stage 3f** — **MOSTLY VALIDATED** (2026-07-12 entries: constants parity
  ...
  **REMAINING:** edge-quality tuning over bright AND dark composite
  ...
  placed) — **REMAINING on the target:** place the model, re-matte the bust
...
- **Stage 3g** — **VALIDATED 2026-07-12** (all ten §19 items PASS on the
  ...
  on the validation record; **FIXED in 5.5g** — `catalog_states.json` was
  fully rewritten to canonical Danbooru tags (`over_shoulder`→`looking back`
  ...
  matter — the UNet-only identity LoRA carries it only weakly; the 5.5c
  render-identity construction gate now prevents new records from having this
  gap). The item-10 hard-kill orphan window (survivor-move → manifest-save)
...
- *(Stage 5.6a + 5.6b build — 2026-07-16)* **Session 1 of Stage 5.6: the §15 fifth format extension + assembler tier ordering — the code `CHARACTER_VOCABULARY_V2.md` needs before any of its data can be authored (1073 tests passing, 1 skipped; +40 over 5.5).** **5.6a format (`app/model/options.py`):** option `"class"` metadata (internal `classes` — Python keyword; `_str_tuple` parse, `to_dict` round-trip); group `tier` (`VALID_TIERS = P0|P1|P2|P3`, `_coerce_tier` — unknown → load-time `OptionFormatError`, the 5.5c widget-enum pattern; decoupled from `order`, which keeps driving form layout); group `visible_when` load-normalized by `_coerce_visible_when` to `{"group": id}` + exactly one predicate (`any: true` — has a value · `in: [ids]` — selection in/intersects · `class: "c"` — a selected option carries the class), with the V2 doc's degrade contract honored literally: absent/unparsable → `None` = always-visible, never an error, even strict — hand-edited junk can only make a group MORE visible; the one structural addition is `_check_visible_when_not_required` (a hidden required group would make creation unsatisfiable — load error on the new-group AND merge paths, all four file-order permutations execution-verified). Both new keys handled by `_new_group` + `_merge_group` (explicit `null` clears; `_apply_file` staged-deepcopy atomicity covers the condition dicts — tested); the existing seven data files load byte-identically (tested: zero errors, all groups untiered/unconditional/classless). **Content gate (§11 Layer-3 as option loading):** `content.gate_open` (**shipped default OPEN — user decision 2026-07-16**; the alternative was offered and declined) read by `content_gate_open` (`is True` — junk or a raising gate reads CLOSED); `CreatorService` gained `gated_option_dirs` + a `gate` callable re-evaluated on EVERY catalog (re)load — dirs run bundled → `data/options` → `app/data/options_gated` (new, README-only) → `data/options_gated`, so the ungated catalog *structurally lacks* the entries, and the assembler consumes the same gate-shaped catalog through the existing `lambda: creator.catalog` (a record holding a gated selection renders without the fragment — a skip, never a filter; tested end-to-end). Option-thumbnail resolution adds the gated dirs to its `resolve_within` bases only while open (containment per-dir — traversal probes clean). **Payload/front-end:** `_option_payload` ships `"class"`, `_group_payload` ships `tier` + `visible_when` (strict-JSON-verified); `creator.js` evaluates conditions against live selections (`condIndex`/`visibleNow`/`selectionChanged`) with a re-render ONLY when a condition-referenced group changes — zero re-renders on a condition-free catalog, numeric-targeted conditions degrade to visible (selections-only semantics), picker search text survives conditional re-renders (and clears on reset/fill-from-record/end-edit); `formGroups()` filters by visibility, so the existing `buildPayload()` visible-set mechanism keeps hidden values out of every round-trip; Settings gained the gate checkbox (`shell.py` `_SETTABLE` + `_BOOL_KEYS`; saves then auto-reloads options via the newly exported `Creator.reloadOptions()`, which now returns true/false so the failure path reports honestly). **5.6b assembler (`app/imagegen/prompt.py`):** `_assembly_groups` — `sorted(catalog.groups(), key=(_TIER_RANK.get(tier, 4), order, id))`, P0→P1→P2→P3→untiered, stable `(order, id)` within a bucket, used by `assemble` only (`assemble_scene` + `_age_fragment` stay flat, lock-tested); an all-untiered catalog collapses to exactly the old flat pass — byte-identity held by direct assertion, an executed monkeypatch A/B (incl. `exclude_groups`/`lead`/`extra`), and the pre-existing exact-positive suite. **The first-window contract test** (`tests/test_tier_ordering.py`) ran UN-skipped on the local real CLIP tokenizer: a V2-shaped maximal record (P0 species + 13 P1 identity groups with Danbooru-register fragments + P2/P3 ballast overflowing the window) keeps every P0/P1 fragment, the quality/subject/adult anchors, and the 6-hex LoRA `lead` trigger below `token_report.boundary_index`, with ballast provably past it. `img:—` = existing `render:false`; `img:gated` = the gated catalog's absence; `img:scene` = `assemble_scene`'s. Scope held: no vocabulary data authored (`app/data/options/` untouched; `options_gated/` README-only), `DECISIONS.md`/`catalog_states.json`/sliders/free-text untouched, Layers 1–4 unchanged. Four authoring notes carried to 5.6c/5.6d (gated files append-only; conditions should reference always-visible groups; numeric-ref degrade; the toggle is a no-op until gated data lands) — see the stage block.
- *(Stage 5.6c build — 2026-07-16)* **Session 2 of Stage 5.6: the Subset A+B render-side vocabulary data — the first V2 data to land on the 5.6a/5.6b machinery (1095 tests passing, 1 skipped; +22 over 5.6a/5.6b).** Data-only: `app/data/options/{00_age,10_identity,20_appearance,30_body,35_species,38_marks,40_anatomy,60_wardrobe}.json` rewritten + `35_species.json`/`38_marks.json` new; `app/data/options_gated/{90_wardrobe_intimate,91_anatomy_intimate,92_piercings_intimate}.json` new; **zero code/blocklist change** (git-diff-confirmed — `options.py`/`prompt.py`/`catalog.py`/`creator.py`/`creator.js`/`app/safety/**`/`catalog_states.json`/`50_personality.json` all untouched; §15's promise that data lands with no rebuild, held). **Subset A:** `race` grown 13→**112** across the 10 V2 families (all legacy ids preserved — `elf`→"High Elf", `alien`→"Alien (Humanoid)"; new ids for the rest) with `class` metadata driving the species blocks; `hybrid_race` (same 112 list, P2, classless — conditions read `race` only); `apparent_age` (8 bands, unambiguous adult phrasing — Layer-1 safe), `age_reality`/`gender_identity`/`pronouns` (render:false); `archetype` grown to 34 and **kind tags→single** (V2 A11). **Subset B:** the frame (`body_type` 12 + `height_band` 7 + `muscle_def` 5 — the slider replacements), skin (28-swatch `skin_tone` + `complexion`), hair (`hair_color` 25 + `hair_color_2` + `hair_color_pattern`[cond hair_color_2] + new `hair_length` 8 + `hair_style` 25 pure shapes + bangs/facial_hair), eyes&face (`eye_color` 17 + `eye_color_2` heterochromia + shape/features/face_shape/lips/nose/eyebrows/makeup), the eight species blocks (`ears`/`horns`/`tail`/`wings` unconditional; fur[beastfolk-mammal]/scale[scaled]/feather[feathered]/chassis+faceplate+seams[construct]/ethereal-opacity+glow[ethereal]/lower_body[monstrous]/elemental_marks[elemental-cosmic]/undead_state[undead], each `visible_when {group:"race", class:…}`), marks/tattoos[cond]/piercings, categorical anatomy (`chest_size`/`waist`/`hips`/`rear`/`body_hair`; the intimate `chest_shape`[cond]/`genitalia`/`genitalia_size`[cond]/`grooming` + `lingerie`/`boudoir_set`/`towel_only`/`nude` on `outfit` + `nipple`/`genital` on `piercings` in the gated files, append-only), and wardrobe (`outfit` **id+kind `multi` pinned** by `catalog.py`, relabeled "Wardrobe", ~85 ungated in the six V2 families + `outfit_palette`/`fit`/`condition`/`neckline`/`accessories` 61/`aesthetic` 24). **Tiers:** P0=`race` only; P1 = the render-identity set + per-class carriers (`fur_color`/`scale_color`/`feather_color`/`ethereal_opacity`/`undead_state`/`chassis_finish`/`lower_body`); `horns`/`wings`/`faceplate` demoted to **P2** (first-window budget — worst UI-constructible single-species record measured at ≤75 content tokens on the real tokenizer); fragments slimmed where they threatened the window (race species tags, `middle aged adult`, de-hyphenated hair/scale terms). **Slider removal** (V2 flag 3): height/weight/muscle groups gone; `age` group kept (bounds only) with its `prompt_ranges` removed (V2 A3 renders via `apparent_age`). **`DECISIONS.md` amended with explicit user approval** — §12 bullet: the height/weight/muscle slider reservation withdrawn ("No continuous sliders … species-relative categorical"); §17 row: "Continuous precision sliders"→"Continuous sliders (any axis — the height/weight/muscle reservation withdrawn at Stage 5.6)". **Two real Layer-1 catches during authoring, fixed data-side (never the blocklists):** `van dyke beard`→`pointed goatee` (the "dyke" slur), and the option↔option adjacency `…hair, red` + `skin-tight…` → reworded `hair_color_2` to "two-tone `<color>` hair"; also `middle-aged`/`nine fox tails`→"aged nine" surfaced and was reworded. **Decisions:** doc open item 3 = keep+lint only (retired values invisibly retained, `validate_against` lints — zero code); O* defaults + cap keys deferred (a later format session); V2 ids adopted with legacy lint-only. **Tests (+22):** new `tests/test_vocabulary_data.py` (14: whole-catalog per-fragment Layer-1 gate-open; a fragment-boundary cross-product prescanned against the blocklists' own term data — the last-word×first-word edges both spaced and zero-sep; `visible_when` referential integrity both directions; required-7 + `outfit` pin; gated structural absence; legacy-record leniency; describe() ships conditions+class as JSON arrays), a real-catalog P0+P1 first-window contract parametrized over lamia/harpy/ghost/android beside the 5.6b fixture, and ~20 existing tests retargeted to the new vocabulary (`hair_style "short"→"bob"` across 5 files, slider-payload tests moved to synthetic drop-in sliders, `fantasy_armor`→`plate_armor`, `distinctive_features`→`marks`, the exact-order assembly test rewritten against the tiered order + no age-range fragment, make_records catalog-clean, `test_bundled_catalog_backward_compatible_untouched_by_5_6a` inverted to assert the V2 tiered/conditional shape). Age-change-alone is now render-irrelevant (renders via `apparent_age`) — tests updated. Layers 1–4 unchanged; the gated-files mechanism is the existing §11 Layer-3 pattern applied to option loading.
- *(Stage 5.6c verification — 2026-07-16)* Ran three parallel adversarial review agents (red-team with executed probes, correctness audit vs `CHARACTER_VOCABULARY_V2.md`, DoD/scope audit), each finding re-executed skeptically with a refutation mandate. **Correctness: 0 real mismatches** — every V2 §3/§4 group/kind/count/option-list verified word-by-word (race 112 all 10 families member-complete, archetype 34, all swatch counts, all conditions keyed correctly, tiers per the Img column, required-7 byte-identical, JSON hygiene clean); 7 arguable class-assignment judgment calls flagged (mothfolk/beefolk as beastfolk-mammal, aliens classless, etc.) — none spec violations. **DoD/scope: PASS** — all 7 DoD items MET by execution (zero code diff, required-7/age-gate/catalog_states/free-text constraints held, suite 1095/1/skip with the first-window tests un-skipped on the real tokenizer, gated structural absence, legacy leniency, DECISIONS byte-exact). **Red-team: 1 finding** — `_gate_adjacency` (in `prompt.py`) excludes free-text pieces from its zero-separator concat pass, so the option↔`appearance_notes` boundary can form a separator-stripped blocked substring (e.g. `…average build` + `yke…`). **Skeptic-triaged and NOT actioned in 5.6c (correct call):** it reproduces with an **empty catalog** (no 5.6c data) — pre-existing `prompt.py` code, not a data regression; the string the model actually receives is comma-joined and **passes Layer 1** (`filter_text("average build, yke…", "prompt").allowed == True`), so it's a defense-in-depth belt gap in the paranoid zero-sep pass, not a rendered-slur path; and fixing it is a `prompt.py` code change, outside this data-only session's scope. **Recorded as a `prompt.py` hardening note** in the 5.6c stage block for a future code session. Suite green: 1095 passing, 1 skipped. **Next: 5.6d — Subset C+D data + the free-text swap.**
- *(Stage 5.6a + 5.6b verification — 2026-07-16)* Ran three parallel adversarial review agents (red-team with executed probes, correctness review, DoD/scope audit), then a skeptic pass re-executing every finding's fix with a refutation mandate. **Red-team: zero reproduced findings** — hostile `class`/`tier`/`visible_when` (JSON `Infinity`/`NaN`, 200k-char strings, RLO unicode, deep nesting) all degrade or land as clean `OptionFormatError`s with `describe()` strict-JSON-safe; gate-closed reach attempts (describe, create payload, assembler, thumbnail traversal via gated `image` paths) all structurally blocked; required×visible_when unrepresentable across all merge orderings; no front-end condition loop is constructible (no synthetic event dispatch); `_apply_file` staging isolates condition-dict mutation. **Four real findings, all fixed and skeptic-CONFIRMED:** (1) CRITICAL — `content.gate_open` was missing from `shell.py`'s `_SETTABLE` allowlist, so the new Settings checkbox was dead on arrival (every write rejected, checkbox reverting; masked because tests wrote `Settings.set` directly) — fixed with the allowlist entry + a `_BOOL_KEYS` type guard (`True == 1` in Python, and the backend reads `is True`, so an int `1` would have silently persisted as *closed*) + an Api-level test driving the real bridge; (2) the Settings gate handler could report "options reloaded" when the reload failed (`reloadOptions` never rejects) — it now returns true/false and the handler checks; (3) a `visible_when` targeting a numeric group could never be satisfied — permanently hiding the conditioned group, inverting the degrade principle — numeric refs now short-circuit to visible and the slider handler's re-render hook is removed (a mid-drag form rebuild would tear the control out from under the pointer); (4) `pickerSearchText` leaked across create/edit sessions — cleared on reset/fill/end-edit, with the save-adopt-edit retention kept deliberately (same in-flight session). The DoD audit passed every 5.6a/5.6b item with evidence (required-7/age-gate/`catalog_states.json`/DECISIONS/safety-layer constraints all untouched; no scope creep beyond the user-approved gate default + Settings toggle) and verified the contract test executes un-skipped here. Suite after fixes: **1073 passing, 1 skipped**; `node --check` + DOM-id cross-check clean (the five pre-existing dynamic-element misses are baseline-identical). **Next: 5.6c — Subset A+B data authoring + the DECISIONS §12 amendment draft (needs explicit user approval).**
- *(Stage 5.6d build — 2026-07-17)* **Session 3 of Stage 5.6: the Subset C+D chat-side (`render:false`) vocabulary + the free-text swap — 1100 tests passing, 1 skipped (+5 over 5.6c).** New data `app/data/options/{50_mind,55_speech,70_life}.json` (64 groups / ~1,247 options); `50_personality.json` **deleted**; the free-text swap in `app/ui/creator.py` + `app/imagegen/prompt.py` + `app/model/character.py` — **the ONLY production code change** (git-diff-confirmed: `options.py`/`catalog.py`/`catalog_states.json`/the 5.6c A+B data files/`options_gated/*`/`app/safety/**`/`creator.js` all untouched; the always-show `companion_name` decision meant zero front-end touch). **Subset C** (`50_mind` C1–C19, `55_speech` C20–C31): the five ordinal temperament axes C1–C5 (`single(5)→segmented`) + C6 `default_mood`; **C7 `traits` reuses the pre-V2 id** (14→71 — old trait selections survive except retired `intellectual`, which lints); C8–C16 (flaws/quirks/vices/values/moral_compass/fears/near_goal/life_dream/lines_never_cross); C17 `intellect_style` + C18 `skills`(72) + **C19 `signature_skill`** = a full-list single picker duplicating C18's 72 ids with `visible_when {group:"skills", any:true}` (`skills` kept unconditioned — the referent must be always-visible, authoring note 2; there is no format widget for options-drawn-from-another-group's-selection, so the doc's "from the C18 selection" renders as the full list, caveat in an `_note`); C20 `voice_timbre` (replaces the retired `voice` group) + the C21–C27 speech block (C24 `verbal_tic` quoted-phrase labels → sanitized ids; **C25 `catchphrase` is a free-text slot, not a group**); C28–C31 emotional profile. **Subset D** (`70_life` D1–D34): setting/roots/locale, standing/reputation/legal_status, D7 `occupation` (~151 across the doc's families), workplace/job_feeling, the backstory block D10–D15 (D13 `defining_events` 52 — the §9 RAG seed; D14 `secrets` mem:guard), daily-life & tastes D16–D23 (D18 `companion` + the always-show `companion_name` free-text slot), the D-v social dispositions D24–D34 (flag 4 — nothing names the user). **The D-vi builder handoff (relationship/how_met/meeting_scene/scene_tone/time_weather/event_seeds/user_persona) is home:builder / Stage-6 §13 — NOT authored** (confirmed excluded). All 64 groups `render:false`, no `tier`, **no `widget`** (kind+count derives to the doc's segmented/chips/picker hints exactly — verified against `derive_widget`, zero mismatches since no C/D option carries `color`); provisional natural-language `prompt` fragments on every option (the 6d persona-injection / §9 RAG-seed material, `Mem` provisional per flag 1, re-cut at 6d). Caps ("cap N") + single-select O* defaults documented in `_note` only — still **unenforced** (no format key; same posture as 5.6c). **The free-text swap:** `FREE_TEXT_FIELDS` → `nickname`(A2)/`signature_note`(B63, renamed from `appearance_notes`)/`catchphrase`(C25)/`companion_name`(D18) — with `name`, the five §10 filtered slots; `_FREE_TEXT_KEYS`/`describe()`/`_check_free_text` all derive from the one constant. `IMAGE_FREE_TEXT_KEYS = ("signature_note",)` — `signature_note` stays the one image-feeding free-text slot (still Layer-1-gated at assembly), the other three kept out of the image by **absence** from the tuple, not a filter. `character.py` `_rehome_legacy_free_text` rehomes a stored `appearance_notes` note → `signature_note` in `from_dict` **before** construction (so it flows through `__post_init__` gating like a native key; `signature_note` wins if both present; the legacy key is dropped so it neither re-serializes nor double-renders) — a legacy record still renders its note; `personality_notes`/`backstory` on old records are left **inert** (never shown, never rendered, dropped on next edit — keep+lint, no scrubbing). **The real Layer-1 surface for render:false vocabulary is the option-id channel** (`character.py` gates selection/tag values in strict `prompt` context on record save), not the fragment scan (render:false fragments never reach assembly — `prompt.py:270`). A whole-C/D pre-scan of every option id AND fragment in `prompt` context surfaced **three catches, all fixed data-side (never the blocklists):** occupation `con_artist`→id `grifter` (the "con" slur substring), origin_story `street_kid`→id `street_urchin` (the "kid" minor term), `only_child`→id `only_one` + fragment "raised without siblings" (the "child" minor term) — the doc-faithful labels ("Con artist"/"Street kid"/"Only child") kept (labels aren't gated), the ids/fragments made safe (the 5.6c `van dyke`→`pointed goatee` precedent). **Count reconciliations** (enumerated list authoritative over the `~`/header count): `quirks` **36**, `occupation` **~151**, `fav_food` **35**. **keep+lint** for the group-id retirement (disposition/voice → `validate_against` lints; `traits` id reused). **Tests (+5, 19 retargeted):** new in `tests/test_vocabulary_data.py` — C/D group-id/count/`render:false` + the C19↔`skills` wiring (id-set equality, `signature_skill` conditioned, `skills` unconditioned); a maximal-C/D record selecting **every** option id that constructs (proving the ~1,247 ids clear the strict prompt-context record gate) and lints clean; every C/D fragment passes Layer-1 in prompt context (hygiene — the flag-8 fragment scan skips render:false groups); a zero-fragment **leak check** (no C/D group id appears as an assembled-image piece source); the legacy `appearance_notes`→`signature_note` migration loads + renders. 19 retargeted for the swap/removal: 2 group-removal breaks (`test_imagegen` `disposition.render`→`traits`; `test_creator` disposition/voice payload dropped), ~40 `appearance_notes`→`signature_note` (incl. the `free_text.*`/`image.prompt.free_text.*` provenance strings), creator-write-path `backstory`/`personality_notes` → surviving keys, the `test_imagegen` non-render test retargeted to real render:false groups (`warmth`/`voice_timbre`), the now-dead tier-exclusion tuple trimmed (render:false short-circuits before it). Record-layer key-agnostic tests (`test_character`/`test_store`/`test_builder_service`) intentionally keep arbitrary free-text keys — the record accepts any `free_text` key by design; only the creator write-path validates the field set. Layers 1–4 unchanged. **Not this session (5.6e / later):** the [HARDWARE] render pass, the carried 5.5b A/B + 3c drills, the D-vi/Stage-6 builder catalogs, the cap/default format keys, the `_gate_adjacency` free-text hardening (still the noted pre-existing belt gap).
- *(Stage 5.6d verification — 2026-07-17)* Ran a **five-stream adversarial workflow** — correctness-vs-doc ×3 (one agent per data file, word-by-word over `CHARACTER_VOCABULARY_V2.md` §5/§6), red-team the free-text swap + the option-id channel, DoD/scope — each finding to be re-executed by a refutation-mandated skeptic. **All five streams returned 0 findings** (nothing to skeptic-verify; ~398k agent tokens of file-reading + executed probes). **Correctness:** every C/D group id/kind and the full option-label set verified against the doc for all 64 groups; C19↔`skills` wiring; C25/`companion_name` confirmed as free-text (not groups); D-vi confirmed excluded. **Red-team (probes executed):** the legacy-mapping precedence (signature_note wins, appearance_notes dropped, personality_notes/backstory inert); no `render:false` C/D fragment reaches the assembled image; every suspect option id (`torture`/`fugitive`/`died_in_another_world`/`a_body_buried`/`brawling`/`seduction`/`nya`/`grifter`…) clears the strict prompt-context record gate; `nickname`/`catchphrase`/`companion_name` never feed the image — **none reproduced**. **DoD/scope:** only `creator.py`/`prompt.py`/`character.py` changed in production code (git-diff), required-7 + age gate + `catalog_states.json` + the 5.6c A+B data untouched, suite green. Corroborated by an independent programmatic pre-scan (all 64 group counts exact — which also rules out duplicate option ids, since a dup would load a lower count — and every id + fragment Layer-1-clean in strict prompt context). Suite green: **1100 passing, 1 skipped**. **Next: 5.6e — the [HARDWARE] render pass** (weak-honor P3 fields kept-or-cut; full V2 re-acceptance; the carried 5.5b A/B + 3c drills).
- *(Stage 5.6e build — 2026-07-17)* **Session 4 of Stage 5.6 — the [HARDWARE] render-quality pass, closing Stage 5.6. Every render-side decision judged on real pixels on this RTX 4070 Super; NET ZERO data/code edits — the only repo change is this doc.** Drove the pipeline headless (in-process `build_services()` → `ImageService`); each diagnostic was a real `create_character → generate_base` at a fixed seed with ONE fragment varied (isolated ablation), presented as labeled PIL contact sheets for the user's kept-or-cut calls. **Open item 7 weak-honor trio — user KEEPS all three:** `face_shape` (P3) and `nose` (P3) are near-no-ops (square/angular/long → the same soft anime face; aquiline/broad → the same minimal anime nose — the illustrious style overrides the tag), kept as harmless record tails; `height_band` extremes (P2) a confirmed no-op on the non-standard naga frame (identical scale across very_short→towering — species-relative, no scale reference in a solo render), kept for the chat-side `mem:inject` physique line. **Both 5.6c demotions CONFIRMED (no re-promotion):** `horns`/`wings`/`faceplate` render prominently at P2 on a detailed demon/android even behind a full P1 identity load (chunked window-1 placement is not a regression; re-promoting to P1 would spend first-window budget for zero visible gain), and `apparent_age` alone renders the age progression (the age-group `prompt_ranges` removal holds; anime style compresses the old end, which restoring can't fix). **Full V2 re-acceptance** on a naga hero "Sylara" (`lamia`, serpent-coil `lower_body` — a non-standard P1-identity frame, the hardest case): create → 6 avatar candidates → reference → bootstrap (batch 64 → 30 proposed, sim 0.92–0.96) → confirm 30 → **LoRA train to completion** (dim 16, 1600 steps, 6-hex trigger `0c3129`, 85 MB) → catalog (20 frames, identity holds across expression×pose) → matte (20/20 clean incl. the serpent coil, `isnet_anime`, 0 BiRefNet escalations — correct coverage routing, 0 purged) → on-demand pose (novel sultry/over_shoulder state, LoRA-steered) → composite over a generated forest background — **image quality user-approved** (5.5-closure discipline; all evidence from the run's own `data/jobs`/`bootstrap.json`/`lora.json`/`catalog.json`/`data/logs/audit-*.jsonl`). **Carried drills cleared:** 5.5b chunking A/B — a maximal V2 record (92 content tokens, boundary at piece 24) rendered same-seed with `image_gen.encode_chunked` true vs false; the tail `signature_note` ("a bright crimson rose", token ~92) renders ONLY chunked and is truncated away otherwise, everything else shared (chunking drops nothing; token accounting validated [HERE] via `clip_token_counter`); 3c minor-recall — cull screened 64 candidates, 0 wrongly blocked (all adult; `test_bootstrap_cull.py::test_real_classifier_recall_against_the_minor_tag_file` proves it catches above-threshold minor-coded tags); 3c blocked-frame purge — matte re-screened 20 frames fail-closed, 0 purged (`test_matte.py` purge anchors prove the path). Suite green throughout: **1100 passing, 1 skipped** (unchanged — no code/data touched; the real-tokenizer first-window contract runs un-skipped here and passes). §12/§17 slider amendment untouched; required-7 + age gate + `catalog_states.json` + all C/D + gated data untouched. The 5.6c `_gate_adjacency` free-text hardening remains the noted pre-existing belt gap (a `prompt.py` code change, out of this render-pass scope). **Stage 5.6 COMPLETE → Next: Stage 6.**
- *(Stage 5.6e verification — 2026-07-17)* Adversarially verified the closure. **DoD/scope:** `git status` was clean before the doc edit and shows only the single intended `BUILD_PLAN.md` change after (zero code/data/blocklist/`catalog_states.json`/required-7/gated/C–D edits — the pass validated the 5.6c authoring rather than changing it); the diagnostic renders + review sheets live entirely under gitignored `data/` (`data/characters/`, `data/builders/`, `data/jobs/`, `data/logs/`). **No-regression:** suite re-run **1100 passing, 1 skipped** post-session, and the real-tokenizer P0+P1 first-window contract (`test_tier_ordering.py`, incl. the real lamia/harpy/ghost/android catalog) ran un-skipped and green — the budget the horns/wings/faceplate P2 demotion protects is intact (no tier changed). **Budget/Layer-1 red-team:** no tier re-promotion happened, so the ≤75-token first-window budget is unchanged; no blocklist or option-id/fragment edit happened, so no Layer-1 surface moved. **Change-log accuracy:** the DONE-HERE claims trace to real artifacts (bootstrap sim 0.92–0.96, train dim16/1600 steps, catalog 20, matte 20/20 with escalated:0 purged:0, chunking A/B tail-only-chunked) and to the user's own visual calls (keep ×3 weak-honor, confirm ×2 demotions, full acceptance approved). **Stage 5.6 COMPLETE. Next: Stage 6.**
