...
stage block). **Session 1 (2026-07-16) shipped 5.6a + 5.6b DONE-HERE (1073 tests
passing, 1 skipped; adversarially verified — 4 findings, all fixed + skeptic-confirmed):**
the §15 fifth format extension (group `visible_when` evaluated front-end with re-render
on selection change; option `"class"` metadata; group `tier` P0–P3 closed enum; the
gated-options directories + `content.gate_open` — **default OPEN, user decision
2026-07-16**, Settings-view toggle with auto-reload, §11 Layer-3 structural absence
governing creator AND assembler through the shared live catalog) and the tier-bucketed
assembly order (P0→P1→P2→P3→untiered, byte-identical on the untiered catalog; the
P0+P1-first-window contract test ran un-skipped on the local real CLIP tokenizer).
**Next: 5.6c — Subset A+B data authoring** (the ~86-race file with `class` metadata, B
groups with canonical fragments, slider removal, the DECISIONS §12 amendment draft for
explicit user approval; see the stage block's authoring notes). 5.6d–5.6e follow.
