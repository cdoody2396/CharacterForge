...
**Current stage:** **Stage 4 — Library & Management** (next to build). Stage 3
— Image Pipeline is **COMPLETE**: 3a–3g all done-here AND hardware-validated
on the RTX 4070 Super 12 GB target, with the named residuals below (none
gating Stage 4/5).
...
**Next action (when resumed):** **begin Stage 4 — Library & Management**
(view/sort/filter/edit; edit → offers regeneration + marks catalog stale
(§14); per-character footprint display — LoRA + catalog + cached frames,
all three sources now real; deletion recommendation + automatic LRU cap
over the 3g `last_used` signal; exact thresholds are the §15 deferred item
resolved there). Stage 4 also owns the **startup reconciliation sweep**
(deferred item below): `catalog.old/` orphans, bootstrap-candidate orphans,
and the 3g cache orphans (`cache.new/` is self-sweeping; a kill between
survivor-move and manifest-save leaves an unrecorded frame pair). The whole
Stage-3 hardware track is validated on the target machine (RTX 4070 Super
12 GB) with a live character on disk (`c517663a…`: reference → 20-frame
vetted set → trained LoRA → 20-frame matted catalog → on-demand cache).
**Residual hardware items (named in the pending flags, none gating Stage
4/5):** 3c Layer-2 false-negative recall check + optional face-swap leg
...
recall check); 3g states-file prompt tuning (drop-in data edit).
