...
Stage 3a — Base generation (**DONE-HERE** 2026-07-10, hardware-validation pending);
Stage 3b — IP-Adapter baseline identity (**DONE-HERE** 2026-07-11, hardware-validation pending);
Stage 3c — Identity bootstrap + auto-filter (**DONE-HERE** 2026-07-11, hardware-validation pending);
Stage 3d — LoRA promotion (**DONE-HERE** 2026-07-11, hardware-validation pending);
Stage 3e — Seed catalog generation (**DONE-HERE** 2026-07-11, hardware-validation pending);
Stage 3f — Matting / keyable output (**DONE-HERE** 2026-07-12, hardware-validation pending).
**Pending hardware-validation flags:**
- **Stage 3a** — the diffusers/SDXL model call, offline config load, VRAM-slot
  swap timing, and coherent-image output validate on the 16 GB target machine
  per `docs/IMAGE_PIPELINE.md` §6.
- **Stage 3b** — `load_ip_adapter` with the pinned ViT-H encoder folder, the
  encoder-footgun diagnostic, steered-render VRAM, base↔identity mode-swap VRAM
  release, scale-band tuning (incl. the age-anchor-holds-at-high-scale safety
  check), and fully-offline steered generation, per `docs/IMAGE_PIPELINE.md` §8.
- **Stage 3c** — the InsightFace/imgutils/inswapper model calls, the
  similarity-floor + Layer-2-classifier calibration on the real checkpoint (the
  safety-critical minor-appearance recall), generate-then-unload VRAM release
  before the cull, and the single-cv2 (`opencv-contrib-python`) + Windows
  `insightface` install, per `docs/IMAGE_PIPELINE.md` §11.
- **Stage 3d** — the kohya `sd-scripts` subprocess: the generated
  `train_config.toml` key contract against the installed sd-scripts revision
  (+ the output-filename suffix tolerance), engine-unloaded-before-training
  VRAM fit at dim 16 / 1024px / fp16 + gradient-checkpointing on the 16 GB
  floor, identity-quality tuning (dim/alpha/steps/lr, no overfit), and the
  `train_failed` timeout/failure paths, per `docs/IMAGE_PIPELINE.md` §13.
- **Stage 3e** — the LoRA-at-generation call (`load_lora_weights` +
  per-generate `cross_attention_kwargs` scale on the unfused kohya SDXL LoRA;
  default scale 1.0 is safe, custom scales are the risk), checkpoint+LoRA VRAM
  fit at 832×1216, the auto-filter's threshold tuning on real pose-varied
  frames (the relaxed `face_area_min` + the similarity floor vs full-body /
  over-shoulder poses), and the matrix render time, per `docs/IMAGE_PIPELINE.md`
  §15.
- **Stage 3f** — the ONNX matting call on the real user-placed
  `isnet-anime.onnx` (the transcribed rembg pre/post constants parity-diffed
  against rembg itself in a throwaway venv), edge quality on real 832×1216
  renders over bright AND dark composite backgrounds (halo tuning via
  `erode_px`/`feather_px`, BiRefNet escalation), the degenerate coverage
  floors vs real portrait / frame-filling cells, putalpha keyable semantics,
  the offline posture, and CPU throughput + idempotence, per
  `docs/IMAGE_PIPELINE.md` §17.
